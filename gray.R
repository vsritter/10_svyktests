...
#   N_pop, pwbeta1, beta2, X, ucp = .7, u.min = 1, u.max = 2, p = 0.7) %>%
#   bind_cols(X, Z) %>%
...
B <- mclapply(1:500, mc.set.seed = T, function(b){
  # dat <- fastcmprsk::simulateTwoCauseFineGrayModel(
  ...
  # b <- seq(0.3, 0.1, length.out = 5)
  # b <- rep(0, 5)
  b <- c(.3,.3,0,0,0)
  
  x1 <- rep(0:1, N_pop*ph[1]*c(.6, .4)); X <- cbind(x1)
  dat1 <- fastcmprsk::simulateTwoCauseFineGrayModel(
    N_pop*ph[1], beta1 = cbind(b[1]), beta2, X, u.min = 1, u.max = 2, p = 0.7) %>% 
    bind_cols(X)
  
  x1 <- rep(0:1, N_pop*ph[2]*c(.6, .4)); X <- cbind(x1)
  dat2 <- fastcmprsk::simulateTwoCauseFineGrayModel(
    N_pop*ph[2], beta1 = cbind(b[2]), beta2, X, u.min = 1, u.max = 2, p = 0.7) %>% 
    bind_cols(X)
  
  x1 <- rep(0:1, N_pop*ph[3]*c(.6, .4)); X <- cbind(x1)
  dat3 <- fastcmprsk::simulateTwoCauseFineGrayModel(
    N_pop*ph[3], beta1 = cbind(b[3]), beta2, X, u.min = 1, u.max = 2, p = 0.7) %>% 
    bind_cols(X)
  
  x1 <- rep(0:1, N_pop*ph[4]*c(.6, .4)); X <- cbind(x1)
  dat4 <- fastcmprsk::simulateTwoCauseFineGrayModel(
    N_pop*ph[4], beta1 = cbind(b[4]), beta2, X, u.min = 1, u.max = 2, p = 0.7) %>% 
    bind_cols(X)
  
  x1 <- rep(0:1, N_pop*ph[5]*c(.6, .4)); X <- cbind(x1)
  dat5 <- fastcmprsk::simulateTwoCauseFineGrayModel(
    N_pop*ph[5], beta1 = cbind(b[5]), beta2, X, u.min = 1, u.max = 2, p = 0.7) %>% 
    bind_cols(X)
  ...
  data.frame(B = b, t(gray.srs), t(gray), t(svy.gray))
}) %>% bind_rows()
# B
